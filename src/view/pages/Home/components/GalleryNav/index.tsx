      ...
      {isMobile ? (
        <Slider {...settings} className="w-full">
          {images.map((image, index) => (
            <div
              key={index}
              className="h-[500px] flex justify-center items-end relative group"
            >
              <div
              ...
              >
                ...
                <Link
                  to="#"
                  className="w-48 h-20  border bg-slate-800/10 rounded-full flex items-center justify-center mb-56"
                >
                  <span className="relative z-10 font-bold text-3xl text-white  transition-transform duration-500 ease-in-out group-hover:translate-y-6">
                    {image.label}
                  </span>
                </Link>
              </div>
        ...
        <div className="flex justify-center items-center">
          <div className="flex flex-wrap flex-row justify-center items-center h-[600px] lg:space-y-0 ">
            {images.map((image, index) => (
              <div
                key={index}
              ...
              >
                <div className="absolute inset-0 bg-black bg-opacity-30 transition-opacity duration-300 ease-in-out group-hover:bg-opacity-0"></div>
                <span className="relative z-10 mb-32 font-bold text-3xl text-white -rotate-90 transition-transform duration-500 ease-in-out group-hover:translate-y-6">
                  {image.label}
                </span>
              </div>
            ))}
